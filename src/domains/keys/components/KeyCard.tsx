...
import type { ApiKeyWithStatus, TokenUsageStats } from "@/types";
...
export default function KeyCard({
  ...
  const formatDate = (date: Date) => {
    return new Date(date).toLocaleDateString(
      t("keyStatus.neverExpires") === "永不过期" ? "zh-CN" : "en-US",
    );
  };
        ...
        case "success":
          return (
            <span className="badge badge-success">
              ...
              {testResult.message ||
                t("common.save", { defaultValue: "Valid" })}
            </span>
          );
        case "error":
          return (
            <span className="badge badge-danger" title={testResult.details}>
              ...
              {testResult.message ||
                t("common.cancel", { defaultValue: "Failed" })}
            </span>
          );
        default:
          return null;
  ...
  return (
    <div
      className={cn("card p-4", apiKey.status === "expired" && "opacity-60")}
    >
      {/* 第一行：标题 + 状态 + 右侧所有操作按钮 */}
      ...
      <div className="mb-2">
        <code
          className={cn(
            "font-mono text-xs text-slate-600 bg-slate-100 px-2.5 py-1.5 rounded-lg block w-full",
